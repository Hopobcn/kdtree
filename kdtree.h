...
struct dimension_extractor {
    ...
    subtract(const Point& p1, const Point& p2, std::size_t dim)
    {
        if (Dimension == dim) {
...
typename boost::geometry::default_distance_result<Point>::type
subtract(const Point& p1, const Point& p2, std::size_t dim)
{
    return dimension_extractor<Point, 0, boost::geometry::dimension<Point>::type::value>::subtract(
        p1, p2, dim);
}
...
private:
    ...
    struct best_match {
        ...
        best_match(const node_ptr& n, double d)
            : node(n),
              distance(d) {}
    };
    ...
    static void
    knearest(const Point& query, const node_ptr& currentNode, size_t k, PriorityQueue& result)
    {
        if (!currentNode) {
            return;
        }
